...
# -------------------------
# Session State for Refresh & Reset
# -------------------------
...
if submit:
    try:
        ...
        st.sidebar.success(f"Flight {fid} added.")
        st.session_state.refresh += 1  # trigger rerender
...
if st.sidebar.button("Reset System"):
    ...
    st.session_state.reset_trigger = True  # trigger rerun

# Trigger refresh if reset was clicked
if st.session_state.reset_trigger:
    st.session_state.refresh += 1
    st.session_state.reset_trigger = False

# Reference refresh counter to force rerender
...
if flights_raw:
    ...
    for row in flights_raw:
        flights_list.append({
            "id": row[0], "airline": row[1], "atype": row[2],
            "arr": pd.to_datetime(row[3]), "dep": pd.to_datetime(row[4])
        })
    ...
    st.dataframe(df_flights[["id","airline","atype","Arrival","Departure"]], use_container_width=True)

    st.write("✈️ **Mark flights as departed:**")
    for f in flights_list:
        col1, col2 = st.columns([4,1])
        with col1:
            st.write(f"Flight {f['id']} ({f['airline']} - {f['atype']})")
        with col2:
            if st.button("Depart", key=f"depart_{f['id']}"):
...
#     for row in flights_raw:
#         flights_list.append({
#             "id": row[0], "airline_id": row[1], "airline": row[2], "atype": row[3],
#             "arr": pd.to_datetime(row[4]), "dep": pd.to_datetime(row[5])
#         })
#     df_flights = pd.DataFrame(flights_list)
#     df_flights["Arrival"] = df_flights["arr"].dt.strftime("%Y-%m-%d %H:%M")
#     df_flights["Departure"] = df_flights["dep"].dt.strftime("%Y-%m-%d %H:%M")
...
#                 db.delete_flight(f["id"])
#                 st.session_state.refresh += 1
#                 st.session_state.depart_triggered = f["id"]  # store which flight was departed
# else:
#     st.info("No flights added yet.")
