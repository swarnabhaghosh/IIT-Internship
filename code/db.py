...
def add_flight(fid, airline, atype, arr: datetime, dep: datetime):
    conn = get_conn()
    c = conn.cursor()
    exists = c.execute("SELECT 1 FROM flights WHERE id=?", (fid,)).fetchone()
    if exists:
        conn.close()
        raise ValueError(f"Flight ID '{fid}' already exists.")
    c.execute(
        "INSERT INTO flights (id, airline, atype, arr, dep) VALUES (?,?,?,?,?)",
        (fid, airline, atype, arr.isoformat(), dep.isoformat())
    )
    conn.commit()
    conn.close()
